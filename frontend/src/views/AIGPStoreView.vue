...
function displayName(name: string) {
  return name.replace(/_aigp$/, '').replace(/_/g, ' ')
}
...
async function loadPrograms() {
  ...
  error.value = ''
  try {
    programs.value = await api<StoreProgram[]>('/aigp-store/programs')
  } catch (e: unknown) {
...
async function downloadProgram(program: string) {
  ...
  try {
    await apiPost('/aigp-store/download', { program })
...
async function installProgram(program: string, agentName: string) {
  ...
  try {
    await apiPost('/aigp-store/install', { program, agent_name: agentName })
...
<template>
  <div class="store-page" @click="showDropdown = null">
    ...
    <p class="page-subtitle">Browse and install AIGP programs from the community</p>

    <div v-if="loading" class="loading">Loading programs from AIGP Store...</div>
    ...
    <div v-else-if="programs.length === 0" class="empty-state">
      <p>No programs available in the AIGP Store yet.</p>
    </div>
