...
- **AIGP 包系统** — 可插拔功能包（*_aigp），热插拔扩展 Agent 能力
...
│   └── SoulBot_Agent/        # 主 Agent (AISOP + AIGP 包路由)
│       ├── agent.py          # Agent 定义
│       ├── main.aisop.json   # AISOP 蓝图 (意图路由)
│       └── aigp/             # AIGP 功能包目录
│           ├── soulbot_chat_aigp/              # 聊天包
│           └── aigp_creator_evolution_aigp/     # Creator 包
├── tests/                    # 1266 单元测试
├── docs/                     # 设计文档与开发者指南
│   └── guide/
...
├── main.aisop.json   # AISOP 蓝图 (意图路由到 AIGP 包)
├── .env              # 配置（模型选择等）
└── aigp/             # AIGP 功能包目录
    └── README.md     # 将 *_aigp 包放在此目录
```
...
### 6.3 AISOP + AIGP 驱动的 Agent

```python
# agent.py — 通过 AISOP 蓝图和 AIGP 包驱动行为
import json, os
...
- **aigp/ 目录** — 可插拔功能包，每个 `*_aigp/main.aisop.json` 是包入口
...
WORKSPACE_DIR=aigp            # AIGP 包目录（相对于 Agent 目录）
```
...
| **AIGP 包** | 功能包目录（`*_aigp/`），每个包含 `main.aisop.json` 入口 |
...
agent.py _dynamic_instruction()
    ...
    └── 注入 [Available AIGP packages] (可用包列表)
    ↓
LLM 按 mermaid 流程执行：
    NLU[匹配意图到 AIGP 包] → Run[读取并执行 *_aigp/main.aisop.json]
    ↓
AIGP 包内的 mermaid 继续执行子流程
    ↓
返回结果给用户
```
...
├── AISOP 驱动     →  main.aisop.json → mermaid 流程 → AIGP 包路由
