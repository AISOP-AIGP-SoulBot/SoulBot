...
**AI Agent Framework with AISOP Protocol and AIGP Package System**
...
SoulBot is a Python-based AI Agent framework that connects to LLMs through CLI subprocesses (ACP protocol) — **no API keys required**. It introduces a unique architecture where agent behavior is defined by **AISOP blueprints** (`.aisop.json`) and extended through **AIGP packages** (`*_aigp`), making AI agent behavior deterministic, reproducible, and version-controlled.
...
- **AIGP Package System** — Hot-pluggable capability packages (`*_aigp`) that extend agent functionality
...
```bash
git clone https://github.com/AISOP-AIGP-SoulBot/SoulBot.git
cd SoulBot
pip install -e .
```
...
| **AIGP Packages** | AI General-purpose Packages — hot-pluggable capability modules (`*_aigp/`) with their own `main.aisop.json` entry points |
...
agent.py → _dynamic_instruction()
    ...
    └── [Available AIGP packages] (capabilities)
    ↓
LLM follows mermaid flow:
    NLU[Match Intent] → Run[Load & Execute *_aigp/main.aisop.json]
    ↓
AIGP package's mermaid flow executes domain-specific logic
    ↓
...
AIGP operates under a tripartite federated trust model:

- **aisop.dev** (Seed Layer): Defines the unchangeable format structure.
- **aigp.dev** (Authority Layer): Defines the evolving governance rules.
...
├── AISOP Engine     →  main.aisop.json → mermaid flow → AIGP routing
...
# Behavior
WORKSPACE_DIR=aigp            # AIGP package directory
...
│   └── SoulBot_Agent/        # Main agent with AIGP packages
...
# From GitHub
pip install git+https://github.com/AISOP-AIGP-SoulBot/SoulBot.git

# One-line run (uv)
uvx --from git+https://github.com/AISOP-AIGP-SoulBot/SoulBot.git soulbot web --agents-dir .
```
...
```
Copyright 2026 AISOP.dev | AIGP.dev | SoulBot.dev
