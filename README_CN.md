...
**基于 AISOP 协议和 AIGP 包系统的 AI Agent 框架**
...
SoulBot 是一个基于 Python 的 AI Agent 框架，通过 CLI 子进程（ACP 协议）连接 LLM —— **无需 API Key**。它引入了独特的架构：Agent 行为由 **AISOP 蓝图**（`.aisop.json`）定义，通过 **AIGP 包**（`*_aigp`）扩展能力，使 AI Agent 的行为变得确定性、可复现、可版本控制。
...
- **AIGP 包系统** — 热插拔功能包（`*_aigp`），即插即用扩展 Agent 能力
...
| **AIGP 包** | AI General-purpose Package — 热插拔功能模块（`*_aigp/`），每个包含 `main.aisop.json` 入口 |
...
agent.py → _dynamic_instruction()
    ...
    └── [Available AIGP packages]（可用能力）
    ↓
LLM 按 mermaid 流程执行：
    NLU[匹配意图] → Run[加载并执行 *_aigp/main.aisop.json]
    ↓
AIGP 包内的 mermaid 流程执行领域逻辑
    ↓
返回结果给用户
```
...
├── AISOP 引擎     →  main.aisop.json → mermaid 流程 → AIGP 包路由
...
│   └── SoulBot_Agent/        # 主 Agent（含 AIGP 包）
├── tests/                    # 1266 单元测试
├── docs/                     # 文档
└── pyproject.toml            # 项目配置
```
